...
Build a dedicated, wall-mountable Hong Kong bus ETA display on the Waveshare ESP32-S3-RLCD 4.2" board. The device fetches real-time arrival estimates from KMB and Citybus open APIs for up to 6 fixed routes (2 pages of 3), renders them on the reflective ST7305 display with large, legible text, and refreshes the display every 10 seconds (on :00/:10/:20/:30/:40/:50 boundaries) while fetching ETA data every ~30 seconds (jittered, configurable) — relaxed to ~300 s during the out-of-service night window (01:00–05:30). No touch, no audio, no OTA — a single-purpose appliance that shows "when's the next bus" at a glance.
...
13. **Temperature & humidity display** — Display the current air temperature (`NN°C`) and relative humidity (`NN%`) in the header band using the Hong Kong Observatory (HKO) "Current Weather Report" open data API (`rhrread`) — a single fetch supplies both. Fetched via the `eta_fetch_task` piggyback at a time-based ~600 s cadence (state-independent — every ~20 cycles in service, every ~2 cycles out of service, so it stays fresh inside its 30-min TTL all night). Hide on failure or stale data (30-min TTL shared by both fields). Station configurable via `routes.json` `weather.station` (default: "Hong Kong Observatory"); the same station governs both fields. If the station has no humidity entry, humidity hides independently while temperature remains visible.
14. **Fetch all routes, all pages** — `eta_fetch_task` fetches ETA data for **every route in `routes.json`** each cycle, regardless of the currently-visible page. Toggling pages therefore always shows fresh ETAs — no waiting for a page-specific fetch, and no immediate refetch is triggered by the toggle (all pages are fetched every cycle anyway). Implementation: see CLAUDE.md §Multi-Page Display.
15. **Out-of-service adaptive fetch interval** — Between `oos_start` (default 01:00) and `oos_end` (default 05:30), if **all** configured routes return empty ETAs (all three ETA slots `-1`), the device is deemed out of service and the ETA fetch interval relaxes to `oos_fetch_interval_seconds` (default 300 s). Service resumes at `oos_end`, or immediately when any route returns a non-empty ETA (data wins over the time window). The OOS wait is capped at the `oos_end` boundary so the morning restore is never delayed. Daytime all-empty (e.g. an API/network outage) does NOT enter out-of-service mode — recovery stays fast. The state is local to `eta_fetch_task`; the display renders naturally (`--`). Config keys + validation: see CLAUDE.md §Display Refresh.
...
- **`eta_fetch_task`** (priority: `tskIDLE_PRIORITY+2`): Owns the ETA fetch loop, Wi-Fi modem-sleep toggling, battery sampling, and **weather fetch** (time-based 600 s cadence). Fetches **all pages' routes** every cycle (FR 14) and evaluates the out-of-service state (FR 15). Cadence specified in CLAUDE.md §Display Refresh (current: `fetch_interval_seconds` ~30 s ±10% jitter in service, `oos_fetch_interval_seconds` ~300 s out of service). Writes fetched ETA values into the **inactive** double-buffer, then atomically flips a shared active-buffer index.
...
| **SNTP failure** | If SNTP sync fails or is delayed beyond the boot window, the system clock still shows the correct time when an RTC value with year ≥ 2026 was restored at boot. Otherwise (RTC year < 2026, first boot, or no RTC) the clock-trust gate hides time/date/ETAs: header time `----`, header date `-- --- (---)`, footer `Connecting...`, ETAs `--`. Normal display resumes once the first sync completes. |

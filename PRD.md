...
> **Document role**: requirements (what/why/constraints — reviewable, testable).
> Implementation details, current values, and file paths live in
> [CLAUDE.md](CLAUDE.md); layout/rendering rules live in [design.md](design.md).
> See the Document Map below for per-topic ownership — a fact belongs in exactly
> one place, and each file points to the owner instead of repeating it.

## Document Map

| Topic | Requirements (this file) | Implementation / current values |
|---|---|---|
| Display init & ST7305 driver | §2 Hardware Constraints, §10 #1/#2 | CLAUDE.md §Waveshare Reference Repository, §ST7305 Register Values |
| Refresh / fetch / OOS cadence | FR 9, FR 15, NFR "Refresh interval" | CLAUDE.md §Display Refresh |
| RTC + clock-trust gate | FR 3, FR 12, NFR "Time source" | CLAUDE.md §PCF85063 RTC |
| zh-HK CJK fonts | §2, §5 | CLAUDE.md §zh-HK CJK Font Support |
| KMB / Citybus endpoints | §4 Data Source, FR 5–6 | CLAUDE.md §Data Source Handling |
| Weather | FR 13 | CLAUDE.md §Weather Temperature & Humidity |
| Multi-page display | FR 14, §6, §8 | CLAUDE.md §Multi-Page Display |
| Wi-Fi reconnect / footer state | FR 7, NFR "Wi-Fi reconnect" | CLAUDE.md §WiFi Connection State |
| Layout zones / rendering rules | FR 7, §5 | design.md §3 |
...
| **Display driver init/timing** | Init sequence derived from the **Waveshare official reference driver** (`../waveshare-reference/02_Example/ESP-IDF/11_U8G2_Test/components/u8g2_st7305/`), which is the authoritative source for this exact board. Do NOT use values from the generic ST7305 datasheet §7.9.2 — the Waveshare values differ significantly (voltage trims, frame rate, duty setting, gate timing, pixel format, display inversion). Register map, pixel-layout strategy, SPI baseline, and reset timing: see CLAUDE.md §Waveshare Reference Repository and §ST7305 Register Values. |
| **Text / font rendering** | U8g2 library in *page-buffer mode* (no full framebuffer allocation). Destination and bus-stop name fields render zh-HK Chinese (e.g. 黃埔花園, 海濱花園總站); `route_config.c` reads `dest_zh`/`stop_zh` from `routes.json` (falls back to `dest_en`/`stop_en` if absent). Font specs (file names, glyph coverage, partition layout): see CLAUDE.md §zh-HK CJK Font Support. The 8 MB PSRAM is *available* but the software shall not *depend* on PSRAM for core function; PSRAM may be used optionally for font caching if needed, but must gracefully degrade if PSRAM init fails. |
...
| **Power** | Primary: battery (fixed installation, not portable; USB Type-C present for charging and firmware flashing). Active power-saving: **light sleep + DFS** (`CONFIG_PM_ENABLE`, idle ~1–3 mA) with Wi-Fi modem-sleep between fetch cycles, plus **in-cycle TLS connection reuse** (7 HTTPS handshakes/cycle → ~3) and **`WIFI_PS_MAX_MODEM` during the out-of-service night window**. All data fetching and render cadences are unchanged (PRD FR 9/15 intact). Deep-sleep intentionally NOT used — the device maintains a persistent Wi-Fi connection, with ETA fetch every ~30 s and display render every 10 s. Tier 2/3 battery optimisation: see §10 decision #10. |
| **RTC** | Onboard PCF85063 RTC chip (I2C addr 0x51, SDA=GPIO13, SCL=GPIO14) used as the persistent wall clock: time is restored from the RTC at boot (before SNTP) and every successful SNTP sync updates the RTC — boot, periodic, and the daily 06:00 resync. Requires a rechargeable RTC backup battery in the PH1.0 holder to retain time across power-off. If the chip is absent or the battery is dead, the firmware degrades to SNTP-only. Clock-trust gate and implementation: see CLAUDE.md §PCF85063 RTC and FR 3. |
...
3. **SNTP time sync** — After obtaining an IP, synchronise the system clock via NTP. Use the HK timezone (UTC+8). The NTP server is **stdtime.gov.hk**. Display "HH:MM" in the dashboard header once sync completes. Before the first SNTP sync, the system clock is restored from the onboard PCF85063 RTC (when valid) so the header shows the correct time immediately and the clock works even if Wi-Fi/SNTP is unavailable. **Clock-trust gate**: if the RTC year is < 2026 (severely unsynced — first boot, dead backup battery, or years of power-off), the clock is not trusted and the header date/time, the footer timestamp, and the ETA values stay hidden until the first successful SNTP sync. A daily NTP resync is triggered at 06:00 local time to correct clock drift accumulated over 24+ hours of uptime (see FR 12). Trust rules, RTC interplay, and failure modes: see CLAUDE.md §PCF85063 RTC.
4. **Load route configuration** — On startup, read `routes.json` from the SPIFFS partition. The file defines up to 3 routes per page (a mix of KMB and Citybus), each with: operator, stop ID, route number, destination label (zh-HK via `dest_zh`, with `dest_en` fallback), and bus-stop name (zh-HK via `stop_zh`, with `stop_en` fallback). If the file is missing or corrupt, show a persistent error message on the display and halt further ETA fetching.
5. **Fetch KMB ETA** — For each KMB route, HTTP GET the KMB route-specific ETA endpoint (see §4). Parse the earliest `eta` and store the raw ISO 8601 timestamp as a `time_t` epoch value — do not precompute minutes at fetch time. When the stop is a terminal where the API returns both directions, filter to keep only the configured destination (case-insensitive `dest_en` comparison). Implementation and filtering detail: see CLAUDE.md §Data Source Handling and §Permanent Never List.
6. **Fetch Citybus ETA** — For each Citybus route, HTTP GET the Citybus route-specific ETA endpoint (see §4). The response already contains only the requested route. Parse the earliest `eta` timestamp and store the raw ISO 8601 timestamp as a `time_t` epoch value — do not precompute minutes at fetch time.
7. **Render dashboard** — On wall-clock boundaries, draw the following zones on the 400 × 300 display (decoupled from the ETA fetch cycle — always renders the last known ETA values). Boundary cadence: see CLAUDE.md §Display Refresh. Zone geometry and typography: see design.md §3. Minutes-remaining are computed fresh at render time from the stored raw epoch timestamps and the current wall-clock time — never from precomputed values stored at fetch time:
    - **Header** (top ~8%): Current local time in `HH:MM` format, large font, plus date and weather (see FR 13).
...
9. **Refresh cadence** — The ETA fetch cycle and the display render cycle are **decoupled**. The display re-renders on wall-clock boundaries so the header clock stays precisely aligned regardless of network/HTTP fetch latency. The render interval is read from `routes.json` `refresh_seconds` and must be a divisor of 60 for clean wall-clock alignment (invalid values are rejected with a warning and clamped to the safe default). The ETA fetch runs on a longer, configurable cadence (`fetch_interval_seconds`), relaxing to `oos_fetch_interval_seconds` when out of service (see FR 15). Always render the last known ETA values — never block the render cycle waiting for a fresh fetch. Current default values and validation rules: see CLAUDE.md §Display Refresh.
...
12. **Daily NTP resync** — Once per day at 06:00 local time, trigger a one-shot SNTP resync to correct clock drift accumulated over 24+ hours of uptime. The resync is blocking but bounded (10 s max retry); if it times out, log a warning and continue normally — do not crash or hang. The "once per day" guard uses day-of-year tracking so the resync fires exactly once per day, with one retry attempt per day regardless of prior success/failure. Every successful sync (boot, periodic, and this 06:00 resync) writes the fresh stdtime.gov.hk time back into the onboard PCF85063 RTC, so the RTC is *updated* from the time server — never "reset" to a hardcoded value (see FR 3).
13. **Temperature & humidity display** — Display the current air temperature (`NN°C`) and relative humidity (`NN%`) in the header band using the Hong Kong Observatory (HKO) "Current Weather Report" open data API (`rhrread`) — a single fetch supplies both. Fetch piggybacks on the existing ETA fetch cycle. Hide on failure or stale data (shared stale TTL). Station configurable via `routes.json` `weather.station` (default "Hong Kong Observatory"); the same station governs both fields. If the configured station has no humidity entry, humidity hides independently while temperature remains visible. Cadence, TTL, fonts, and overlap drop order: see CLAUDE.md §Weather Temperature & Humidity.
14. **Fetch all routes, all pages** — `eta_fetch_task` fetches ETA data for **every route in `routes.json`** each cycle, regardless of the currently-visible page. Toggling pages therefore always shows fresh ETAs — no waiting for a page-specific fetch, and no immediate refetch is triggered by the toggle. Implementation: see CLAUDE.md §Multi-Page Display.
15. **Out-of-service adaptive fetch interval** — Inside the out-of-service window (`oos_start`–`oos_end`), if **all** configured routes return empty ETAs (all three ETA slots `-1`), the device is deemed out of service and the ETA fetch interval relaxes to `oos_fetch_interval_seconds`. Service resumes at `oos_end`, or immediately when any route returns a non-empty ETA (data wins over the time window). The OOS wait is capped at the `oos_end` boundary so the morning restore is never delayed. Daytime all-empty (e.g. an API/network outage) does NOT enter out-of-service mode — recovery stays fast. The state is local to `eta_fetch_task`; the display renders naturally (`--`). Default window and interval values + validation: see CLAUDE.md §Display Refresh.
...
- **Layout zones**: See FR 7 and design.md §3 for zone definitions, proportions, typography, and visual rules. This PRD defines *what* to display; design.md is the source of truth for *where* and *how it looks*.
- **Readability targets**:
...
- **zh-HK font handling (WORKING — verified on device)**: Destination and bus-stop names render in zh-HK Chinese. ASCII fallback retained for pure-ASCII strings. Implementation details (font files, glyph coverage, partition table, data flow): see CLAUDE.md §zh-HK CJK Font Support.
...
- **No stock U8g2 constructor exists for 400×300**. The **Waveshare
  ...
  authoritative starting point. Our project should adopt this
  driver as the baseline, adapting it to our landscape-optimised
  layout if needed. Pixel-layout strategy, CASET/RASET, SPI baseline,
  and rotation: see CLAUDE.md §Waveshare Reference Repository and
  §ST7305 Register Values.
...
- **`eta_fetch_task`** (priority: `tskIDLE_PRIORITY+2`): Owns the ETA fetch loop, Wi-Fi modem-sleep toggling, battery sampling, and the weather fetch. Fetches **all pages' routes** every cycle (FR 14) and evaluates the out-of-service state (FR 15). Cadence: see CLAUDE.md §Display Refresh. Writes fetched ETA values into the **inactive** double-buffer, then atomically flips a shared active-buffer index.
- **`display_task`** (priority: `tskIDLE_PRIORITY+3`): Owns wall-clock render boundary alignment, the daily NTP resync, the page toggle (KEY button, FR 14), and `render_dashboard()`. It always reads from the **active** double-buffer and never waits for fresh data. Render boundaries: see CLAUDE.md §Display Refresh.

### Shared data: double-buffering (no mutex)

Two full `route_data_t[MAX_PAGES][ROUTES_PER_PAGE]` buffers (`s_route_buf[2][MAX_PAGES][ROUTES_PER_PAGE]`) plus an atomically-swapped active-buffer index (`s_active_buf_idx`, 0 or 1). No mutex or semaphore is used:
...
| Weather fetch + battery sampling (piggyback) | `eta_fetch_task` |
...
| Page toggle (KEY button) | `display_task` |
...
|---|---|
| **Refresh interval** | ETA fetch and display render are decoupled. Display re-renders on wall-clock boundaries (interval configurable via `routes.json` `refresh_seconds`, must be a divisor of 60) so the header clock stays aligned. ETA fetch runs at `fetch_interval_seconds`, relaxing to `oos_fetch_interval_seconds` when out of service (FR 15). Never block the render cycle waiting for a fetch — always render the last known ETA values. Current default values: see CLAUDE.md §Display Refresh. |
...
| **Wi-Fi reconnect** | On disconnect: show "Connecting..." in the footer band (replaces "Updated HH:MM:SS"), retain last known ETAs for up to 3 minutes then expire to "--". Reconnect automatically (infinite retry, 1 s backoff between attempts). "Connecting..." persists until successfully reconnected — no auto-dismiss. Implementation: see CLAUDE.md §WiFi Connection State. |
| **SNTP failure** | If SNTP sync fails or is delayed beyond the boot window, the system clock still shows the correct time when a trusted RTC value (year ≥ 2026) was restored at boot. Otherwise the clock-trust gate hides time/date/ETAs: header time `----`, header date `-- --- (---)`, footer `Connecting...`, ETAs `--`. Normal display resumes once the first sync completes. Trust rules and failure modes: see FR 3 and CLAUDE.md §PCF85063 RTC. |
| **Power / battery** | Battery-powered (fixed install, not portable). Active power-saving: **light sleep + DFS** (idle ~1–3 mA; CPU 160→80 MHz when idle, held at 160 MHz during fetch bursts), Wi-Fi modem-sleep between fetch cycles, **`WIFI_PS_MAX_MODEM` during the out-of-service night window**, and **in-cycle TLS connection reuse** (fewer handshakes). ETA fetch cadence (FR 9/15), weather cadence, and the 10 s full-buffer redraw are all unchanged — nothing is paused or slowed. The full display buffer is re-sent every render cycle (no partial-window optimization). Deep-sleep is NOT used — the device stays connected to Wi-Fi. Implementation and measured-vs-estimated numbers: `docs/plan-battery-optimizations.md` and CLAUDE.md §Power Management. |
...
The following were previously flagged as open questions and have been resolved as follows. Current numeric values referenced here live in CLAUDE.md; each decision records the rationale and the outcome.

| # | Question | Decision |
|---|---|---|
| 1 | ST7305 init sequence | **Waveshare official reference driver** (`../waveshare-reference/02_Example/ESP-IDF/11_U8G2_Test/components/u8g2_st7305/`) is the authoritative source. Do not use generic datasheet §7.9.2 values. Register map: see CLAUDE.md §ST7305 Register Values. |
...
| 8 | **Larger zh-HK fonts (dest + stop)** | **Implemented 2026-07-26** — Noto Sans CJK HK Bold 24px (dest) + Regular 20px (stop) via otf2bdf + bdfconv. Factory partition resized to 8 MB. Font specs: see CLAUDE.md §zh-HK CJK Font Support. |
| 9 | **Fetch-all pages + out-of-service adaptive interval** | **Implemented 2026-08-16** — All routes in `routes.json` are fetched every cycle regardless of the visible page (FR 14). Out-of-service window (default 01:00–05:30) with all routes empty relaxes the fetch interval to 300 s (FR 15); any non-empty ETA restores service. Configurable via `routes.json` (`fetch_interval_seconds`, `oos_fetch_interval_seconds`, `oos_start`, `oos_end`). See CLAUDE.md §Display Refresh and `docs/plan-fetch-all-oos.md`. |
| 10 | **Battery optimisation without function/frequency reduction** | **Implemented 2026-08-24, verified on device 2026-08-25** — Automatic light sleep + DFS (`CONFIG_PM_ENABLE` + `FREERTOS_USE_TICKLESS_IDLE`; idle ~1–3 mA, CPU 160→80 MHz idle / 160 MHz burst via a CPU_FREQ_MAX lock), in-cycle TLS connection reuse (KMB/Citybus/HKO persistent handles; ~7→3 handshakes/cycle), and `WIFI_PS_MAX_MODEM` during the OOS night window (listen interval 5 beacons). KEY button converted to a level-triggered ISR + light-sleep wakeup so page-toggle still works while asleep. USB flashing/monitoring preserved via `CONFIG_USJ_NO_AUTO_LS_ON_CONNECTION` + a 30 s boot grace. **No function, fetch cadence, or render cadence is changed.** On-device verification: power consumption greatly reduced and USB flashing/monitoring confirmed working (2026-08-25). Estimated ~5–6× runtime improvement on the 26–30 h measured baseline (est. ~4.5–8 days) — see `docs/plan-battery-optimizations.md`. |

...
static void st7305_full_init(u8g2_st7305_t *dev)
{
    ...
    ESP_ERROR_CHECK_WITHOUT_ABORT(st7305_write_cmd(dev, 0x20));
...
static uint8_t u8g2_st7305_display_cb(u8x8_t *u8x8, uint8_t msg, uint8_t arg_int, void *arg_ptr)
{
    ...
    case U8X8_MSG_DISPLAY_DRAW_TILE: {
        ...
        /* FIX: U8g2 already provides tile->tile_ptr pointing at the start
         * of the current 8-px row within this tile. The prior expression
         * `tile->tile_ptr - x_pos*8` read up to 392 bytes before the
         * actual buffer for any x_pos>0 tile, producing garbage source
         * data for the per-column LUT pack and corrupting multi-character
         * CJK glyphs. */
        /* FIX: Convert global pixel columns to local byte offsets within this tile.
         * tile->tile_ptr points to the current tile's 8-byte row data. Global pixel
         * columns must be converted to byte offsets (0-7) within this tile. */
        uint8_t *row_base = tile->tile_ptr;
        int tile_byte_offset = first_col;  /* Byte offset of this tile within the row */
        ...
        for (int sr = 0; sr < 4; sr++) {
            ...
            for (int col = addr_first_col; col <= addr_last_col; col += 4, idx++) {
                /* Convert global pixel column to local byte offset within this tile */
                int local_col = col - tile_byte_offset;
                all_rows[idx] = st_lut[0][(row_base[local_col] >> shift) & 3]
                              | st_lut[1][(row_base[local_col + 1] >> shift) & 3]
                              | st_lut[2][(row_base[local_col + 2] >> shift) & 3]
                              | st_lut[3][(row_base[local_col + 3] >> shift) & 3];
            }
        }
...
/* (voltage-profile switching removed — see HANDOFF.md for history) */
